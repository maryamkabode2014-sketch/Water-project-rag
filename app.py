...
def answer_question(vectorstore, question: str, hf_token: str, model_name: str, top_k: int):
    ...
    system_prompt = (
        "شما یک دستیار متخصص تحلیل گزارش‌های فنی آب و خاک هستید. "
        "فقط بر اساس متن‌های زیر که از یک سند PDF استخراج شده‌اند پاسخ بده. "
        "اگر پاسخ در متن موجود نیست، صادقانه بگو که اطلاعات کافی در سند وجود ندارد. "
        "پاسخ را به زبان فارسی روان، دقیق و ساختاریافته بنویس و در صورت لزوم به شماره صفحه مرتبط اشاره کن.\n\n"
        f"متن سند:\n{context}"
    )
